...
static absl::StatusOr<std::unique_ptr<llvm::Module>> ExtractKernels(
...
static void AddConfigToModule(llvm::Module* module, llvm::StringRef config) {
  constexpr llvm::StringRef kName = "xla_backend_extra_options";
  llvm::LLVMContext& context = module->getContext();
  llvm::MDString* config_string = llvm::MDString::get(context, config);
  llvm::MDNode* config_node = llvm::MDNode::get(context, {config_string});
  module->getOrInsertNamedMetadata(kName)->addOperand(config_node);
}
...
CpuCompiler::CompileCpuExecutable(std::unique_ptr<HloModule> module) {
  ...
  if (module->config().debug_options().xla_cpu_use_thunk_runtime()) {
    ...
    using Config = llvm::StringRef;
    using Kernel = llvm::StringRef;
    absl::flat_hash_map<Config, absl::flat_hash_set<Kernel>>
        extra_config_to_kernels;
    for (const auto& k : ir_emitter2.kernels()) {
      if (k.backend_extra_options.empty()) continue;
      auto [_, inserted] =
          extra_config_to_kernels[k.backend_extra_options].insert(k.name);
      CHECK(inserted) << "Kernel " << k.name << " is not unique";
      num_extra_functions++;
    }
    const int num_extra_parts = extra_config_to_kernels.size();
    // We assign one dylib to each set of kernels that have the same config.
    // This is reasonable because we assume very few kernels will set
    // extra options, and if they do, the options are likely to be identical.
    ...
    if (num_extra_parts > 0) {
      ...
      for (const auto& [config, kernels] : extra_config_to_kernels) {
        TF_ASSIGN_OR_RETURN(std::unique_ptr<llvm::Module> new_module,
                            ExtractKernels(llvm_module.get(), kernels));
        AddConfigToModule(new_module.get(), config);
