...
// Returns an HloModuleConfig with its .debug_options.xla_backend_extra_options
// set by the values embedded in the LLVM module. The rest of the fields
// of the proto should be ignored since they're just the default value.
// We could instead return an unordered_map<str, str>, but we already have
// helpers to process these extra options that expect a DebugOptions, so this
// ends up being simpler.
static absl::Nullable<std::unique_ptr<HloModuleConfig>>
GetXlaBackendExtraOptions(const llvm::Module& module) {
  constexpr llvm::StringRef kName = "xla_backend_extra_options";
  llvm::NamedMDNode* named_node = module.getNamedMetadata(kName);
  if (named_node == nullptr) return nullptr;
  CHECK_EQ(named_node->getNumOperands(), 1);
  llvm::MDNode* node = named_node->getOperand(0);
  CHECK_EQ(node->getNumOperands(), 1);
  llvm::Metadata* md = node->getOperand(0);
  llvm::MDString* md_string = llvm::cast<llvm::MDString>(md);
